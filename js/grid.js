// Logique exacte de Mygamify (extraite de arrowwords.js)
// DEF à (x,y), lettres placées selon dir:
// RB: col x+1, lignes y à y+size-1   (vertical, col droite)
// RR: ligne y,  cols x+1 à x+size    (horizontal, même ligne)
// BR: ligne y+1, cols x à x+size-1   (horizontal, ligne suivante)
// BB: col x,   lignes y+1 à y+size   (vertical, même col)
...
// Retourne les coordonnées (col, row) des cases lettres d'un mot
// selon la logique exacte de arrowwords.js Mygamify
export function getLetterCells(w) {
  const cx = parseInt(w.x), cy = parseInt(w.y);
  const cells = [];
  for (let j = 0; j < w.size_word; j++) {
    let col, row;
    if (w.dir === "RB") { col = cx + 1;     row = cy + j;     }
    if (w.dir === "RR") { col = cx + 1 + j; row = cy;         }
    if (w.dir === "BR") { col = cx + j;     row = cy + 1;     }
    if (w.dir === "BB") { col = cx;         row = cy + 1 + j; }
    cells.push({ col, row, pos: j });
  }
  return cells;
}

// axe du mot: RR/BR = horizontal (x), RB/BB = vertical (y)
export function getAxe(dir) {
  return (dir === "RR" || dir === "BR") ? "x" : "y";
}
